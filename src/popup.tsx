...
async function renderResults(jufoID: string) {
  var detailsResponse = await fetch(
    "https://jufo-rest.csc.fi/v1.1/kanava/" + jufoID
  );
  if (detailsResponse.ok) {
    ...
    if (detailJson.length > 0) {
      ...
      document.getElementById("link")!.innerHTML =
        '<a target="_blank" href="https://jfp.csc.fi/#!PublicationInformationView/id/' +
        jufoID +
        '">Open in JUFO portal</a>';
    } else {
      document.getElementById("error")!.innerText =
        "Could not retrived data for channel " + jufoID;
    }
  } else {
      ...
      async (results) => {
        ...
        if (selected.length > 0) {
          ...
          var searchResponse = await fetch(
            "https://jufo-rest.csc.fi/v1.1/etsi.php?issn=" + selected
          );
          if (searchResponse.ok) {
            ...
            var searchResponse = await fetch(
              "https://jufo-rest.csc.fi/v1.1/etsi.php?nimi=" + selected
            );
            handleSearchResponse(searchResponse);
          }
          showResults();
